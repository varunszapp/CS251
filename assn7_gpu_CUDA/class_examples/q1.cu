...
{
      int i = (blockDim.x * blockIdx.x * blockDim.y) + (blockDim.x * threadIdx.y) + threadIdx.x ;
      if(i >= num)
           return;
...
int main(int argc, char **argv)
{
    ...
    dim3 threadspblock(num_row,num_col);
    ...
    blocks = num /(num_row * num_col);

    if(num % (num_row * num_col))
           ++blocks;

    calculate<<<blocks, threadspblock>>>(gpu_mem, num);
